...
bool AIPilot_TeamName::Init(void* pInitData, unsigned short index)
{
	...
	else  //蓝方
	{
		...
		fout<< _pInitData->m_dInitAlt << "|"
			...
			<< _pInitData->m_fInitPitch << "|"
			<<- _pInitData->m_fInitYaw << ", "
		    << "Type=Air+FixedWing,Name=F-16C bl.50,Color=Blue" << std::endl;
	}
...
void AIPilot_TeamName::write2acmi()
{
	...
	fout.precision(14);
	fout<< "T=" << m_Input.m_AircraftMoveInfo.m_dSelfLon << "|"
		<< m_Input.m_AircraftMoveInfo.m_dSelfLat << "|";
	fout.precision(6);
	fout<< m_Input.m_AircraftMoveInfo.m_dSelfAlt << "|"
		<< m_Input.m_AircraftMoveInfo.m_fRoll / PI * 180 << "|"
...
bool AIPilot_TeamName::Step()
{
	...
	if(m_Input.m_AircraftBasicInfo.m_lTimeStamp < 10000)
		m_Output.m_FCCtrlCmd.m_eMainTaskMode = Enum_AircraftTaskMode_WVR;
	else
	{
